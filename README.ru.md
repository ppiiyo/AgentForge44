...
## ⚙️ Техническая архитектура и функциональные модули

Платформа **KostromAi44** спроектирована как высокодоступная отказоустойчивая среда для визуального моделирования, отладки и запуска мультиагентных систем в промышленном масштабе. Ниже представлены ключевые технические модули и особенности реализации:

### 🚀 1. Передовой топологический планировщик задач
*   **Динамическое построение графа зависимостей**: При запуске `PipelineExecutor` анализирует топологию направленного ациклического графа (DAG) для выявления независимых параллельных ветвей вычислений.
*   **Параллелизация через асинхронные пулы**: Независимые параллельные узлы выполняются одновременно с помощью оптимизированного пуллинга промисов Node.js, что снижает суммарную задержку выполнения пайплайна до 60% по сравнению с последовательным LLM-оркестрированием.
*   **Прогрессивный проброс данных**: Переменные и промежуточные структуры выходных payload-ов родительских узлов автоматически форматируются и интерполируются в шаблоны mustache (`{{variable}}`) последующих шагов.

### 🔄 2. Циклы самокоррекции (Self-Correction & Auto-Healing)
*   **Детерминированная валидация по метрикам**: Узел рецензирования (`Reviewer`) автоматически проверяет и ранжирует ответы вышестоящих агентов на соответствие заданным правилам (регулярные выражения, соответствие JSON-схеме, семантический скоринг).
*   **Откат по графу с обратной связью**: В случае недостижения пороговой оценки система автоматически откатывает состояние выполнения на указанный родительский узел, передавая сгенерированную критику рецензента в качестве контекста для исправления.
*   **Предохранитель от бесконечных циклов**: Конфигурируемые лимиты бюджета попыток (retry budget) блокируют выполнение пайплайна при зацикливании, предотвращая бесконечную отправку платных API-запросов.

### 🛡️ 3. Изолированная песочница VM (Tool/Code)
*   **Безопасная изоляция в потоках**: Пользовательские JavaScript-скрипты в узлах `Tool/Code` запускаются внутри защищенных контекстов VM, полностью изолированных от операционной системы хоста, что исключает возможность несанкционированного доступа к файловой системе или сетевым сокетам.
*   **Лимиты ресурсов CPU и памяти**: Рантайм песочницы накладывает строгие ограничения по таймаутам и объему используемой памяти, предотвращая блокировку потоков и атаки типа "Отказ в обслуживании" (DoS).

### 📚 4. Парсинг бинарных файлов и 3D-визуализация RAG
*   **Парсеры бинарных документов**: Системы извлечения данных в автоматическом режиме обрабатывают файлы **PDF (.pdf)**, **Word (.docx)**, **Markdown (.md)** и текстовые документы, разбивая их на семантические блоки (chunks).
*   **Интерактивный каталог блоков**: Удобный визуальный интерфейс позволяет выполнять глубокий полнотекстовый поиск по базе векторов с подсветкой совпадений и выводом метрик схожести (similarity score).
*   **3D WebGL обозреватель эмбеддингов**: Проецирует базу векторных эмбеддингов в интерактивное трехмерное пространство WebGL для визуального контроля качества кластеризации, семантического расстояния и структуры документов.

### 👥 5. Совместное редактирование и распределенные блокировки
*   **WebSocket-коллаборация высокого разрешения**: Полнодуплексный обмен данными через Socket.io координирует курсоры разработчиков на холсте, активные границы выделений и изменения параметров в реальном времени.
*   **Распределенный Lock Manager**: Защищает данные от перезаписи. Выделение или редактирование параметров узла мгновенно блокирует элемент для других участников сессии, отображая карточку активного редактора.

### ⚖️ 6. Отладчик времени (Time-Travel Debugger) и снимки состояний
*   **Инкрементальный аудит истории**: Архитектура непрерывно записывает структуру графа и значения всех промежуточных переменных, формируя линейную историю ревизий.
*   **Визуальный Diff-сравнитель**: Позволяет сравнивать версии пайплайнов бок о бок с цветовой индикацией изменений (добавленные, удаленные, измененные узлы и связи).
*   **Изменение памяти на лету**: Вы можете поставить выполнение графа на паузу, вручную изменить промежуточную переменную прямо в отладчике, и эти изменения мгновенно распространятся по остальной цепочке вычислений для симуляции исправленного сценария.

### 🎛️ 7. Интеграция Model Context Protocol (MCP)
*   **Динамическое управление серверами MCP**: Подключайте, авторизуйте и настраивайте удаленные или локальные MCP-сервера напрямую из панели синхронизации.
*   **Протокол интеграции инструментов**: Декларативно синхронизирует схемы инструментов с агентами на холсте, предоставляя им доступ к базам данных, системным операциям и браузерной автоматизации Puppeteer под строгим контролем политик безопасности.
