...
| 📚 **RAG 索引与预览器** | 高级文档分块器与交互式多文档视觉目录，深度支持 PDF & Word (.docx) 等二进制文件，提供文本全文检索与块高亮预览。 |
...
| 🔌 **动态 MCP 编排** | 通过可视化面板直接挂载、配置第三方 Model Context Protocol (MCP) 服务（如 SQLite、文件系统、Puppeteer 自动化等），授权 AI 智能体跨网络使用系统工具。 |
| ⏱️ **调试帧状态实时修改** | 允许在时间旅行调试器（Snapshot Debugger）内实时修改、覆盖中间计算变量，改动即时向后传导更新画布节点渲染。 |
| 🔔 **全局网络故障 Toast 拦截** | 挂载底层 `fetch` 拦截中间件，自动捕获并拦截 API 网络崩溃及运行故障，并显示带有三种语言绑定的动画 Toast 浮层。 |
