...
export function generateConfigFiles(graphId: string, graphName: string, provider: 'vercel' | 'railway' | 'fly', config: DeploymentConfig) {
  ...
  const standAloneCode = `// Autogenerated KostromAi44 Graph Runner API Entrypoint
...
`;
  ...
  if (provider === 'vercel') {
    const vercelJson = {
      ...
      env: {
        AGENTFORGE_API_KEY: config.apiKeyAuth ? "@kostromai44_api_key_secret" : ""
      }
    };
    ...
    const flyToml = `app = "kostromai44-${graphId}"
